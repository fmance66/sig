...
export default function InformeItemsIvaPage() {
  ...
  async function buscar() {
    ...
    setLoading(true);
    try {
      const res = await api.getItemsIva({ modulo, empresa: empresa.id, periodo });
      setFilas(res.data.resultado);
    } catch {
      toast.current.show({ severity: 'error', summary: 'Error', detail: 'No se pudo obtener el Informe de Ítems' });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="page-iva">
      ...
      <div className="page-header-row">
        <BotonVolver />
        <h2 className="page-title"><i className="fa-solid fa-box" /> Informe de Ítems</h2>
      </div>
      ...
      <DataTable
        ...
        footer={filas.length > 0 && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '2rem', fontWeight: 700, fontSize: '0.82rem' }}>
            <span>Importe: {money(totalizar('importe'))}</span>
      ...
      >
        ...
        <Column field="comprobantes" header="Comprob." style={{ width: '100px' }} />
        <Column body={r => cantidad(r.cantidad)} header="Cantidad" style={{ width: '110px' }} />
        <Column body={r => money(r.importe)} header="Importe" style={{ width: '130px' }} />
