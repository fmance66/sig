...
import * as api from '../../api/ivaCondicionesVenta';
...
export default function CondicionVentaPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await api.getCondicionesVenta(empresa.id);
      setRegistros(res.data.resultado);
    } catch {
      toast.current.show({ severity: 'error', summary: 'Error', detail: 'No se pudo cargar el listado de condiciones de venta' });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSave() {
    ...
    try {
      if (editMode) {
        await api.updateCondicionVenta(editId, empresa.id, form);
        toast.current.show({ severity: 'success', summary: 'OK', detail: 'Condición de venta actualizada' });
      } else {
        await api.createCondicionVenta({ empresa: empresa.id, ...form });
        toast.current.show({ severity: 'success', summary: 'OK', detail: 'Condición de venta creada' });
      }
  ...
  function handleDelete(row) {
    confirmDialog({
      message: `¿Está seguro de eliminar la condición de venta "${row.descripcion}"?`,
      ...
      accept: async () => {
        try {
          await api.deleteCondicionVenta(row.id, empresa.id);
          toast.current.show({ severity: 'success', summary: 'OK', detail: 'Condición de venta eliminada' });
          load();
        } catch {
  ...
  const tableHeader = (
    <div className="table-toolbar my-2">
      ...
      <Button label="Agregar condición" icon="fa-solid fa-plus" size="small" onClick={openNew} />
    </div>
  );
  ...
  return (
    <div className="page-iva">
      ...
      <div className="page-header-row">
        <BotonVolver />
        <h2 className="page-title"><i className="fa-solid fa-handshake" /> Condición de Venta</h2>
      </div>
      ...
      <DataTable
        ...
        header={tableHeader}
        emptyMessage="No hay condiciones de venta registradas"
        size="small"
        stripedRows
        removableSort
      >
      ...
      <Dialog
        ...
        header={editMode ? 'Modificar condición de venta' : 'Agregar condición de venta'}
        footer={dialogFooter}
        style={{ width: '600px' }}
        modal
        draggable={false}
