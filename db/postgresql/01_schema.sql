...
CREATE TABLE IF NOT EXISTS sld_formulario_recibo (
    ...
    -- NULL = motor de cajas x/y (parámetros). No NULL = este diseño no se interpreta con
    -- ese motor: pdfInformes.js lo arma con el layout fijo correspondiente —
    -- LEY_27802 -> reciboLey27802.js (Anexo III, Decreto 407/2026), IPROFESIONAL ->
    -- reciboIprofesional.js. Ver sección 13 de MIGRACION_BITACORA.md.
    modelo_fijo       VARCHAR(20) CHECK (modelo_fijo IN ('LEY_27802','IPROFESIONAL')),
...
-- modulo: 'sueldos' | 'configuracion' | 'seguridad' | 'contabilidad' — validado
-- en código (constante MODULOS en el backend), sin CHECK constraint, para no
-- exigir un ALTER el día que se sumen módulos nuevos (ej. iva).
...
-- -----------------------------------------------------------------------------
-- 16. CONTABILIDAD (BASE)
-- (antes migrations/005_contabilidad_base.sql, plegado acá con el mismo
-- criterio que la sección 14 — primera etapa: Plan de Cuentas, Centros de
-- Costo, Leyendas, Ejercicios. Asientos/Informes/Ajuste por Inflación quedan
-- para una etapa posterior, igual que la migración de los datos legacy cnt_*.)
-- -----------------------------------------------------------------------------
