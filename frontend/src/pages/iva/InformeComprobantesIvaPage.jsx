...
export default function InformeComprobantesIvaPage() {
  ...
  async function buscar() {
    ...
    setLoading(true);
    try {
      const res = await api.getComprobantesIva({ modulo, empresa: empresa.id, periodo });
      setFilas(res.data.resultado);
    } catch {
      toast.current.show({ severity: 'error', summary: 'Error', detail: 'No se pudo obtener el Informe de Comprobantes' });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="page-iva">
      ...
      <div className="page-header-row">
        <BotonVolver />
        <h2 className="page-title"><i className="fa-solid fa-file-invoice" /> Informe de Comprobantes</h2>
      </div>
      ...
      <DataTable
        ...
        footer={filas.length > 0 && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '2rem', fontWeight: 700, fontSize: '0.82rem' }}>
            <span>Comprobantes: {totalizar('comprobantes')}</span>
            <span>Neto: {money(totalizar('neto'))}</span>
            <span>IVA: {money(totalizar('iva'))}</span>
            <span>Total: {money(totalizar('total'))}</span>
          </div>
        )}
      >
        <Column field="tipo_descripcion" header="Tipo de Comprobante" />
        <Column field="comprobantes" header="Cant." style={{ width: '90px' }} />
        <Column body={r => money(r.neto)} header="Neto" style={{ width: '130px' }} />
        <Column body={r => money(r.exento)} header="Exento" style={{ width: '130px' }} />
        <Column body={r => money(r.nogravado)} header="No Gravado" style={{ width: '130px' }} />
        <Column body={r => money(r.iva)} header="IVA" style={{ width: '130px' }} />
        <Column body={r => money(r.total)} header="Total" style={{ width: '130px' }} />
      </DataTable>
    </div>
  );
}
