...
function createCatalogoModel(tableName, columns, numericColumns = [], options = {}) {
  ...
  const selectCols = [`${idColumn} AS id`, ...columns].join(', ');
  const orderClause = hasOrden ? `orden NULLS LAST, ${idColumn}` : idColumn;
  ...
  async function list() {
    const { rows } = await pool.query(`SELECT ${selectCols} FROM ${tableName} ORDER BY ${orderClause}`);
    return rows;
  }

  async function getById(id) {
    const { rows } = await pool.query(`SELECT ${selectCols} FROM ${tableName} WHERE ${idColumn} = $1`, [id]);
    return rows[0] ?? null;
  }

  async function create(data) {
    const extraCols = columns.filter(c => data[c] !== undefined);
    const cols = [idColumn, ...extraCols];
    const vals = [normalize(idColumn, data.id), ...extraCols.map(c => normalize(c, data[c]))];
  ...
  async function update(id, data) {
    ...
    const { rows } = await pool.query(
      `UPDATE ${tableName} SET ${sets.join(',')} WHERE ${idColumn} = $1 RETURNING ${selectCols}`, vals
    );
    return rows[0] ?? null;
  }
