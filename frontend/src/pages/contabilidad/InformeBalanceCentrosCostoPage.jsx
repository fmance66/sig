...
export default function InformeBalanceCentrosCostoPage() {
  ...
  async function buscar() {
    ...
    setLoading(true);
    try {
      const res = await api.getBalanceCentrosCosto({ empresa: empresa.id, ejercicio });
      setCentros(res.data.resultado);
    } catch {
      toast.current.show({ severity: 'error', summary: 'Error', detail: 'No se pudo obtener el Balance de Centros de Costo' });
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="page-contabilidad">
      ...
      <div className="page-header-row">
        <BotonVolver />
        <h2 className="page-title"><i className="fa-solid fa-scale-balanced" /> Balance de Centros de Costo</h2>
      </div>
      ...
      <DataTable
        ...
        size="small"
        stripedRows
        emptyMessage={ejercicio ? 'No hay centros de costo registrados' : 'Elegí un ejercicio'}
      >
