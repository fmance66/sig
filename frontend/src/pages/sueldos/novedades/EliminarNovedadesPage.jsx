...
export default function EliminarNovedadesPage() {
  ...
  function handleEliminar() {
    if (!registros.length) {
      toast.current.show({ severity: 'warn', summary: 'Atención', detail: 'No hay novedades para eliminar con este filtro' });
      return;
    }
    confirmDialog({
      message: `Se eliminarán ${registros.length} novedad(es) que matchean el filtro. ¿Confirma?`,
  ...
  return (
    <div className="page-novedades">
      <Toast ref={toast} />
      <ConfirmDialog />
      <h2 className="page-title"><i className="fa-solid fa-trash" /> Eliminar Novedades</h2>

      <div className="filtros-toolbar">
        <div className="form-field">
          <label>Empleado</label>
          <FiltroTexto name="empleado" value={filtro.empleado} onChange={handleFiltroChange} />
        </div>
        <div className="form-field">
          <label>Tipo de Novedad</label>
          <FiltroTexto name="tipoNovedad" value={filtro.tipoNovedad} onChange={handleFiltroChange} />
        </div>
        <div className="form-field">
          <label>Fecha</label>
          <Calendar value={fecha} onChange={e => setFecha(e.value)} dateFormat="dd/mm/yy" showIcon showButtonBar />
        </div>
